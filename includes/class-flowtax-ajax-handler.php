...
class Flowtax_Ajax_Handler {
    ...
    public static function handle_delete_post() {
        ...
        if ($post_id > 0) {
            ...
            if ($result) {
                wp_send_json_success(['message' => 'Registro eliminado con éxito.']);
            } else {
                wp_send_json_error(['message' => 'No se pudo eliminar el registro.']);
            }
        }
        wp_send_json_error(['message' => 'ID de registro inválido.']);
    }
    ...
    public static function handle_save_form() {
        ...
        if (!empty($errors)) {
            wp_send_json_error(['message' => 'Por favor, corrige los errores.', 'errors' => $errors]);
            return;
        }
